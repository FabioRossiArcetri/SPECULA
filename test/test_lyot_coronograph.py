...
class TestLyotCoronograph(unittest.TestCase):
    ...
    def test_psf_with_and_without_coronograph(self, target_device_idx, xp):
        ...
        # With coronograph: central 2 lambda/D obstruction
        lyot_coro = LyotCoronograph(
            simul_params=self.simul_params,
            wavelengthInNm=self.wavelength_nm,
            iwaInLambdaOverD=2,
    ...
    def test_s0_scaling_with_coronograph(self, target_device_idx, xp):
        ...
        # Test without coronograph
        ef_nocoro = self.get_coro_field(lyot_nocoro, ef)
        s0_no_coro = ef_nocoro.S0

        # S0 with coronograph should be less than without coronograph
        self.assertLess(s0_with_coro, s0_no_coro, "S0 should decrease with coronograph!")
